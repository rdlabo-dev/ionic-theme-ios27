...
For the ordinary Native UI Shell, only iOS-mode components with the theme variables installed are eligible. Vertical Bars back navigation is the exception described below. `ionic-theme-disabled`, `ios-theme-disabled`, and the legacy `ios26-disabled` on an element or ancestor always exclude it. A disabled theme on one tab/segment item keeps its whole group on the Web.
