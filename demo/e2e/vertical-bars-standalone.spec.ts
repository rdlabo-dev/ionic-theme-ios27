...
for (const mode of ['ios', 'md'] as const)
  test(`Vertical Control Area works in ${mode} mode with Ionic CSS and no iOS 27 theme`, async ({ page }) => {
    await page.setViewportSize({ width: 700, height: 900 });
    await page.goto(`/main/index/native-ui-shell?verticalBarsOnly=1&ionicMode=${mode}`);
    await expect(page.locator('ion-app')).toHaveClass(new RegExp(`\\b${mode}\\b`));
    await page.evaluate(() => {
